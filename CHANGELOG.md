...
- `tls.acme.domains` entries are additionally validated against what a public CA
  can issue for via TLS-ALPN-01: ASCII LDH labels only (no wildcards — those need
  DNS-01 — no underscores, and no raw Unicode; supply punycode `xn--` for IDN), no
  leading/trailing hyphen, and no trailing dot. A non-issuable name now fails
  `--check` rather than starting the proxy and only then failing the ACME order
  with no usable certificate.
