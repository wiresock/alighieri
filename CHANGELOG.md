...
- Tokens after a rule's closing `}` are now a parse error instead of being
  ...
  selector typed *outside* the braces — e.g. `socks pass { to: 0.0.0.0/0 }
  command: connect` — was dropped, leaving the rule broader than written (here,
  any command rather than just `connect`). A second rule crammed onto the same
  line is likewise rejected now; comments after `}` are still fine.
