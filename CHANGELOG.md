...
- rustls is updated to 0.23.45 (RUSTSEC-2026-0285).
...
- The Linux release-archive helper now selects its bundled, version-matched
  `alighieri` binary automatically, so normal install and upgrade commands no
  longer need `--binary ./alighieri`; an explicit `--binary PATH` still wins.
- Linux `upgrade` now recognizes exact, unmodified systemd units generated by
...
- During `upgrade`, customized systemd units and drop-ins are never migrated or
  overwritten; incompatible effective settings still fail closed, with
  accurate inspection and recovery guidance instead of assuming a drop-in
...
- `tls.acme.domains` entries are additionally rejected at `--check` when they have
  a shape a public CA clearly cannot issue for via TLS-ALPN-01: each must be a
  multi-label DNS name — not an IP address, a single-label/local name like
...
- The stop/shutdown docs no longer describe the stop as a "graceful stop", which
  ...
  service guide now says so explicitly.
