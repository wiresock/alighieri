...
| ~~**Machine-readable management CLI**~~ | — | — | **Shipped** — versioned capability discovery and JSON user management, with password records on stdin and effective-config targeting. It remains a local CLI/stdio surface for an existing authenticated SSH connection, not a listener, daemon, or fleet API. |
...
| ~~**macOS as first-class**~~ | — | — | **Shipped** (console) — macOS CI, Darwin release archives, and an example `launchd` plist. Homebrew packaging and Apple notarization remain follow-ups. *BSD is still not CI-covered. |
